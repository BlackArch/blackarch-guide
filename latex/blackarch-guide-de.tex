...
Das Toolset wird mittel eines inoffiziellem Benutzer Repositories verteilt, so dass man BlackArch auf einer existierenden Arch Linux Installation installieren kann. Pakete können individuell oder ueber Kategorien installiert werden.
...
Jedes Paket muss mindestens zu blackarch gehören, es wird aber auch viele beziehungen ueber mehrere pakete die zu mehreren Gruppen gehoeren geben.
...
Die blackarch gruppe ist die basis-Gruppe zu der alle Pakete gehören muessen. 
Das ermöglicht es den Nutzern einfach alle Pakete zu installieren.

Was sollte hier drin sein: Alles.

\subsubsection{blackarch-anti-forensic}
Pakete die dazu benutzt werden, forensische Aktivitäten zu umgehen. Das beinhaltet Verschluesselung, Steganographie 
und alles was es ermöglicht Datei/Ordner Attribute zu manipulieren.
Das alles beinhaltet tools die allgemein veränderungen an einem System durchfuehren mit dem Zweck,
...
Eine Obergruppe fuer ähnliche Pakete.

Beispiele: canri, dnsrecon, netmask

\subsubsection{blackarch-reversing}
Übergruppe fuer jegliche decompiler, 
...
Das primäre git repo fuer BlackArch befindet sich hier:
...
\item blackman - Verhält sich aehnlich wie pacman, baut aber aus git. 
	(Nicht zu verwechseln mit nrz's Blackman)
\item check-groups - Überprueft groups.
\item checkpkgs - Überprueft Pakete auf Fehler.
...
\item deptree - Erzeugt einen Abhängigkeitsbaum, der nur blackarch Pakete enthaelt.
\item get-blackarch-deps - Liefert eine List von blackarch Abhängigkeiten fuer ein Paket.
...
\\\\
Jeder ist willkommen. Alle Beiträge werden geschaetzt.
...
Standardmässig sollte git direkt auf origin pushen, aber stelle sicher das deine git konfiguration
...
Wenn du nicht commiten kannst, könntest du mehr erfolg mit
