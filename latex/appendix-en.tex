%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                              %
% BlackArch Linux Guide Appendix                                      %
%                                                                              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tabularx}{\linewidth}{lX}
		\textbf{Q:} & Is BlackArch Linux the right pentesting distribution for me? \vspace{0.2cm} \\
		\textbf{A:} & BlackArch is a Linux pentesting distribution based on ArchLinux. If you're not familiar with ArchLinux, or Linux in general, we strongly suggest you avoid BlackArch due to the learning curve for new users. \vspace{0.7cm} \\
		\textbf{Q:} & Where do I start with BlackArch? \vspace{0.2cm} \\
		\textbf{A:} & You must first get an ISO on the \href{https://www.blackarch.org/downloads.html}{Downloads} page and install it by following the instructions of the installation script. You can find a tutorial to show the process step by step at this URL \href{https://www.blackarch.org/blackarch-install.html}{Blackarch-Installation}. If you encounter any problems and need help, the best place to ask is in our \href{irc://irc.freenode.net/blackarch}{\#blackarch (Freenode)}.
		\vspace{0.5cm} \\
		\textbf{Q:} & Is BlackArch up to date? \vspace{0.2cm} \\
		\textbf{A:} & BlackArch is constantly being updated and offers the latest packages available on Github. We release a new ISO four times a year (quarterly). These new images contain packages that are up to date, and usually include bug fixes. If you find any package that is outdated and wish to see it up to date, please report it as an issue on our   \href{https://github.com/BlackArch/blackarch}{Github} repository.
		\vspace{0.5cm} \\
		\textbf{Q:} & How can I fetch/install the latest update available? \vspace{0.2cm} \\
		\textbf{A:} & By simply running  \texttt{pacman -Syyu --needed --overwrite '*' blackarch}.  This command requires root privileges. \vspace{0.5cm} \\
\end{tabularx}
\newpage
\begin{tabularx}{\linewidth}{lX}
		\textbf{Q:} & Why do I get invalid keyring signature? \vspace{0.2cm} \\
		\textbf{A:} &  It could happen for a wide range of reasons. Below you will find a few suggestions.
		\begin{itemize}
			\item You don't have an internet connection (as you can imagine, a rare problem and can be verified quickly).
			\item You may have a DNS problem, that can't resolve pgp.mit.edu accordingly. Please check your DNS settings.
			\item You may have a network issue, different from the above one, which is hard for us to help since it can be a myriad of things. For example: DNS caching.
			\item You may have a clock/time issue.
			\item You may have something blocking your communication with mit.edu server, for instance: a firewall.
			\item If you're connected through a VPN, try to temporarily disable it and run \texttt{strap.sh} again.
			\item \url{pgp.mit.edu} could be down for some reason (yes, that can happen). See options number 2 and 3 below for more information.
		\end{itemize}
		After testing all the items described above if you still have problems using \texttt{strap.sh}, try the options below::
\end{tabularx}
\textbf{First option:}
\begin{lstlisting}
...
\begin{lstlisting}
	...
	# sh strap.sh
\end{lstlisting}
Just keep in mind that the IP address above is the current IP address of pgp.mit.edu but it can change at any time. Make sure to check before running the command. Also sha1sum should match with the information on \href{https://blackarch.org/downloads.html}{Downloads}.\\
\vspace{0.5cm}
\textbf{Third option:}\\
If the option number 2 didn't solve your issue, try the following on a new downloaded \texttt{strap.sh} file:
\begin{lstlisting}
	# curl -O https://blackarch.org/strap.sh
	# chmod +x strap.sh
	# sha1sum strap.sh
	...
	# sh strap.sh
\end{lstlisting}
Also sha1sum should match again with the information on \href{https://blackarch.org/downloads.html}{Downloads}.
It's very important to follow the suggestions above as well as checking the Archlinux Wiki pages to assist you as needed. If you still encounter any problems, pay us a visit at \href{irc://irc.freenode.net/blackarch}{\#blackarch (Freenode)}.

\begin{tabularx}{\linewidth}{lX}
	\textbf{Q:} & Where can I get help for a problem that I'm facing? \vspace{0.2cm} \\
	\textbf{A:} & Depending on the problem you're facing, you can visit our Github and submit an issue on our Issue page, such as:
	\begin{itemize}
		\item \href{https://github.com/BlackArch/blackarch-site/issues}{BlackArch Website Repository}: related to our website. For example: If a link is broken or an image isn't loading.
		\item \href{https://github.com/BlackArch/blackarch/issues}{BlackArch Repository}: related to our packages. For example: a package hasn't been updated for a while or failed to run.
		\item \href{https://github.com/BlackArch/blackarch-installer}{BlackArch Installer Repository}: related to our installer. For example: the installation failed or you can not boot after a successful installation.
	\end{itemize}
You can also take some time to browse our \href{https://github.com/BlackArch}{other repositories}.
If you still cannot find a solution to your problem, visit our \href{irc://irc.freenode.net/blackarch}{IRC channel} oder \url{https://discord.gg/xMHt8dW}{Discord} and ask for help. But please be advised, BlackArch users are in different parts of the globe (different time zones). Therefore, be patient. Ask your question and wait for a reply.
	\vspace{0.7cm} \\
	\textbf{Q:} & I would like to help. What can I do? \vspace{0.2cm} \\
	\textbf{A:} & BlackArch Linux is a huge project, we are adding new applications and features everyday. If you would like to help us with anything, visit our \href{irc://irc.freenode.net/blackarch}{IRC channel} or \url{https://discord.gg/xMHt8dW}{Discord}. Just remember to wait for a reply, we are in different time zones.
	\vspace{0.7cm} \\
\end{tabularx}
\subsection{Authors}
...
\end{itemize}
The following people have contributed directly to ArchPwn,
and have joined BlackArch::
\begin{itemize}
